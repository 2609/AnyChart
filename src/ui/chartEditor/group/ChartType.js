...
anychart.ui.chartEditor.group.ChartType.isReferenceValuesPresent = function(values, model) {
  ...
  for (var i = 0, count = values.length; i < count; i++) {
    ...
    for (var j = 0, m = model.dataMappings.length; j < m; j++) {
      ...
      var presentInArrayMapping = mapping['getArrayMapping']() != goog.global['anychart']['data']['Mapping']['DEFAULT_ARRAY_MAPPING'] &&
          mapping['getArrayMapping']()[name];
      var presentInObjectMapping = mapping['getObjectMapping']() != goog.global['anychart']['data']['Mapping']['DEFAULT_OBJECT_MAPPING'] &&
