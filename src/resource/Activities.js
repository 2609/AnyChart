 ...
 * @return {*}
 */
anychart.resourceModule.Activities.prototype.resolveOption = function(names, state, dataObj, normalizer) {
  ...
  var dataObjStateVal = dataObjState ? dataObjState[names[0]] : void 0;
  var val = dataObjStateVal || dataObj[names[state]];
  if (goog.isDef(val)) {
    val = normalizer(val);
  } else {
    val = stateObject.getOwnOption(names[0]);
    if (!goog.isDefAndNotNull(val)) {
      val = stateObject.getThemeOption(names[0]);
      if (goog.isDef(val))
        val = normalizer(val);
    }
  }
